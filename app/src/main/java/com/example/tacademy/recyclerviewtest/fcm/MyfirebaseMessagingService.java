...
public class MyfirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        // 메시지를 받았는데 지금 채팅중이었다!!! -> ChatRoomActivity 객체가 현재 화면이면 됨
        // 메모리 상에 로드되어 있던지 -> 이하 무시
        if (U.getInstance().isChattingRoomInside()) {
            // 채팅룸에 현재 머물고 있는데, 푸시가 날라왔다.
            return;
        }
        // 메시지 파싱
        ChatPushModel cpm = new Gson().fromJson(remoteMessage.getData().toString(), ChatPushModel.class);
...
/*
    ...
    // 안테나 역할
    // ex) 카톡 알림 받을지 안받을지
    private void sendNotification(ChatPushModel cpm) {
        // 노티 번호
        ...
        intent.putExtra("FCM", cpm);    // 푸시내용을 액티비티로 전달
        ...
        NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(this)
                ...
                .setContentTitle(cpm.getNickname()+"님 메세지")     //제목
                .setContentText(cpm.getMsg())        //내용
